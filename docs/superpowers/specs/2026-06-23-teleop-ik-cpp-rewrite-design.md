...
[TeleopIKNode]
  ...
  ├─ on_target_with_input(TargetPoseWithInput)
  ...
  │     └─ solve_ik(target_pos, q_seed) → publish arm trajectory
  ├─ on_gripper(Float64)
  │     └─ [0, 1] → [GRIPPER_LOWER, GRIPPER_UPPER] に写像して publish
  └─ on_joint_states(JointState)
        └─ q_current を更新
  │
  ▼
...
/joy (sensor_msgs/Joy)
  ...
  ▼
[GamepadTeleopNode]
  ├─ on_joy(Joy)         … 最新の Joy を保持
  └─ timer_callback()    … 50 Hz 周期
...
| `test_ik_node.cpp` | `clamp_joints`, `solve_ik`, `on_target_with_input` のスティック積分 | URDF 制限, 収束/非収束, スティック dt 積分, セッションリセット, deadzone, max delta |
...
`test_ik_node.cpp` の `on_target_with_input` 系テストは
...
`test_ik_node.cpp` の `on_target_with_input` 系テストでは,
