...
### LAN 内の ROS 2 通信 (Zenoh ブリッジ)

macOS Podman (applehv) は `vmnet-bridged` に非対応のため、DDS マルチキャストが LAN に届かない。
[zenoh-bridge-ros2dds](https://github.com/eclipse-zenoh/zenoh-plugin-ros2dds) でコンテナ内 DDS と LAN 上の ROS 2 を中継する。

```bash
# LAN 側デバイス (Linux + ROS 2): Zenoh ルーターとしてブリッジを起動
zenoh-bridge-ros2dds  # デフォルトで tcp/0.0.0.0:7447 で待受

# macOS: ZENOH_PEER を指定してコンテナ起動 (ブリッジが自動起動)
ZENOH_PEER=tcp/192.168.1.10:7447 ./podman/run.sh
```

- コンテナ内の `ROS_AUTOMATIC_DISCOVERY_RANGE` は `LOCALHOST` に設定される (DDS はコンテナ内に閉じる)
- LAN デバイス側にも `zenoh-bridge-ros2dds` のインストールが必要
