...
def generate_launch_description():
    ...
    def build_nodes(context):
        ...
        so101_urdf_sim = os.path.join(lerobot_description_dir, "urdf", "so101.urdf.xacro")
        so101_urdf_hw = os.path.join(lerobot_description_dir, "urdf", "so101_hw.urdf.xacro")

        is_sim_val = LaunchConfiguration("is_sim").perform(context)
        urdf_path = so101_urdf_sim if is_sim_val == "True" else so101_urdf_hw
